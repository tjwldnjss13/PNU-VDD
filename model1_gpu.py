...
class Model1:
    def __init__(self, n_class=10, learning_rate=.001):
        self.learning_rate = learning_rate
        self.n_class = n_class
        self.n_hidden = 2
        self.conv1 = torch.zeros((4, 8, 8)).cuda()
        self.conv2 = torch.zeros((8, 4, 4)).cuda()
        self.dense = torch.zeros(8 * 4 * 4).cuda()
        self.classifier = torch.zeros(n_class).cuda()
        self.W_conv1 = torch.normal(0, .01, (4, 3, 3)).cuda()
        self.W_conv2 = torch.normal(0, .01, (8, 3, 3)).cuda()
        self.W_dense = torch.normal(0, .01, (8 * 4 * 4, 10)).cuda()
    ...
    def backward(self, input, output, predict):
        ...
        D_dense = torch.zeros(self.dense.shape).cuda()
        ...
        D_W_conv2 = torch.zeros(self.W_conv2.shape).cuda()
            ...
            for n in range(self.W_conv2.shape[2]):
                for c1 in range(self.conv1.shape[0]):
                    for c2 in range(self.conv2.shape[0]):
                        D_W_conv2[c2, m, n] = torch.sum(self.learning_rate * D_conv2[c2] * self.conv1[c1, m:m + self.conv2.shape[1], n:n + self.conv2.shape[2]])
        ...
        W_conv2_flip = torch.flip(self.W_conv2, [1, 2])
        D_conv2_pad = self.padding(D_conv2, (D_conv2.shape[1] + 3, D_conv2.shape[2] + 3), 'zero')
        D_conv1 = torch.zeros(self.conv1.shape).cuda()
        for c1 in range(self.conv1.shape[0]):
            for c2 in range(self.conv2.shape[0]):
                for i in range(self.conv1.shape[1]):
                    for j in range(self.conv1.shape[2]):
                        D_conv1[c1, i, j] = torch.sum(D_conv2_pad[c2, i:i + W_conv2_flip.shape[1], j:j + W_conv2_flip.shape[2]] * W_conv2_flip[c2])
        ...
        D_W_conv1 = torch.zeros(self.W_conv1.shape).cuda()
        for m in range(self.W_conv1.shape[1]):
            for n in range(self.W_conv1.shape[2]):
                for cx in range(x_.shape[0]):
                    for c1 in range(self.conv1.shape[0]):
                        D_W_conv1[c1, m, n] = torch.sum(self.learning_rate * D_conv1[c1] * x_[cx, m:m + self.conv1.shape[1], n:n + self.conv1.shape[2]])
                        # for i in range(self.conv1.shape[1]):
                        #     for j in range(self.conv1.shape[2]):
    ...
    def padding(x, shape, mode='same'):
        if x.shape[1] < shape[0]:
            ...
            for i in range(dif * 2):
                if mode == 'same':
                    pad_ = x[:, 0, :].unsqueeze(1) if i % 2 == 0 else x[:, -1, :].unsqueeze(1)
                    # pad_ = np.expand_dims(pad_, 1)
                elif mode == 'zero':
                    pad_ = torch.zeros((x.shape[0], 1, x.shape[2])).cuda()
                x = torch.cat([pad_, x], 1) if i % 2 == 0 else torch.cat([x, pad_], 1)
        if x.shape[2] < shape[1]:
            dif = shape[1] - x.shape[2]
            for i in range(dif * 2):
                if mode == 'same':
                    pad_ = x[:, :, 0].unsqueeze(2) if i % 2 == 0 else x[:, :, -1].unsqueeze(2)
                    # pad_ = np.expand_dims(pad_, 2)
                elif mode == 'zero':
                    pad_ = torch.zeros((x.shape[0], x.shape[1], 1)).cuda()
                x = torch.cat([pad_, x], 2) if i % 2 == 0 else torch.cat([x, pad_], 2)
    ...
    def maxpool(x, kernel=(2, 2), stride=2):
        ...
        out_ = torch.zeros((x.shape[0], h_out, w_out)).cuda()

        for c in range(x.shape[0]):
            for h in range(0, out_.shape[1]):
                for w in range(0, out_.shape[2]):
                    out_[c, h, w] = torch.max(x[c, 2 * h:2 * h + kernel[0], 2 * w:2 * w + kernel[1]])

        return out_

    @staticmethod
    def relu(x):
        return torch.nn.ReLU(True)(x)
    ...
    @staticmethod
    def softmax(x):
        up = torch.exp(x + 1e-19)
        down = torch.exp(x + 1e-19).sum()

        return up / down

    @staticmethod
    def softmax_derv(x):
        matrix = torch.zeros((len(x), len(x)))
